...
public class UserEditController implements Initializable {
    ...
    public void onCommitPressed() {
        checkAndFill(nameField, nameField1);
        checkAndFill(lastField, lastField1);
        checkAndFill(emailField, emailField1);
        checkAndFill(classField, classField1);
        ...
        if(UserDAO.edit(oldUser, newUser)){
            infoLabel.setText("Successfully edited");
        }else{
            infoLabel.setText("something went wrog");
        }
        updateTable();
    }
    ...
    private void checkAndFill(TextField oldText, TextField newText) {
        if (newText.getText().equals("")) newText.setText(oldText.getText());
    }
    ...
    public void onAddPressed(){
        try {
            Stage stage = (Stage) close.getScene().getWindow();
            Parent parent = FXMLLoader.load(getClass().getResource("/Admin/userAddControl.fxml"));
            Scene scene = new Scene(parent);
            stage.setScene(scene);
            stage.show();
        } catch (IOException e) {
